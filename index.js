...
var RNDraftView = (function (_super) {
    ...
    function RNDraftView() {
        ...
        _this.checkSuggestionChange = function (suggestions) {
            ...
            return suggestions.reduce(function (pre, s) { return pre + s.email; }, "");
        };
        _this.executeScript = function (functionName, parameter) {
            if (_this.webViewRef.current) {
                _this.webViewRef.current.injectJavaScript("window." + functionName + "(" + (parameter ? "'" + parameter + "'" : "") + ");true;");
            }
        };
        _this.onMessage = function (event) {
            var _a = _this.props, onToChange = _a.onToChange, onCcChange = _a.onCcChange, onBccChange = _a.onBccChange, onSubjectChange = _a.onSubjectChange, onSugTextChange = _a.onSugTextChange, onEditorChange = _a.onEditorChange, onActiveStyleChange = _a.onActiveStyleChange, editPosition = _a.editPosition, onSizeChange = _a.onSizeChange;
            var _b = JSON.parse(event.nativeEvent.data), type = _b.type, data = _b.data;
            if (type === "isMounted") {
                _this.widgetMounted();
                return;
            }
        ...
        _this.widgetMounted = function () {
            var _a = _this.props, placeholder = _a.placeholder, defaultValue = _a.defaultValue, to = _a.to, cc = _a.cc, bcc = _a.bcc, from = _a.from, subject = _a.subject, _b = _a.showHeader, showHeader = _b === void 0 ? false : _b, _c = _a.onEditorReady, onEditorReady = _c === void 0 ? function () { return null; } : _c;
        ...
        _this.onAddAtomicBlock = function (type, params) {
            _this.executeScript("onAddAtomicBlock", JSON.stringify({ type: type, params: params }));
        };
        ...
        _this.setSpecialType = function (command) {
            _this.executeScript("toggleSpecialType", command);
        };
        _this.addImage = function (src) {
            _this.onAddAtomicBlock("image", { src: src });
        };
    ...
    RNDraftView.prototype.UNSAFE_componentWillReceiveProps = function (next) {
        if (this.checkSuggestionChange(next.suggestions) !==
            this.checkSuggestionChange(this.props.suggestions)) {
            this.executeScript("setSuggestions", JSON.stringify(next.suggestions || []));
        }
    };
    RNDraftView.prototype.render = function () {
        var _a = this.props.style, style = _a === void 0 ? { flex: 1 } : _a;
        var htmlPath = "file://" + RNFS.MainBundlePath + "/assets/node_modules/" + Package.name + "/index.html";
        return (React.createElement(WebView, { ref: this.webViewRef, style: style, source: { uri: htmlPath }, keyboardDisplayRequiresUserAction: false, originWhitelist: ["*"], onMessage: this.onMessage }));
    };
    return RNDraftView;
}(Component));
